...
Load( const std::string& file_name, int buffer_count )
{
    ...
    // サウンドのリソースハンドル生成
    m_ResourceHandle = LoadSoundMemFromSoftSound( m_SoftSoundHandle, buffer_count );
...
void 
aqua::core::CSoundResource::
SetCurrentPlayTime(float set_time)
{
    if (m_ResourceHandle == AQUA_UNUSED_HANDLE)
        return;

    SetSoundCurrentTime(static_cast<LONGLONG>(set_time * 1000.0f), m_ResourceHandle);
}
