 ...
 *  @version    7.17
 *
 *  Copyright (c) 2013-2022, Kazuya Maruyama. All rights reserved.
 */
...
{
    if (m_Sound.IsEnable())
        PlaySoundMem(m_Sound.GetResourceHandle(), (m_LoopFlag ? DX_PLAYTYPE_LOOP : DX_PLAYTYPE_BACK), 0);
}
...
void
aqua::CSoundPlayer::
SetCurrentPlayTime(float new_time)
{
    m_Sound.SetCurrentPlayTime(new_time);
}
